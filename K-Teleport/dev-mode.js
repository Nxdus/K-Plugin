...
fs.watch(path.join(__dirname, 'target', 'K-Teleport-1.0.jar'), (eventType, filename) => {
...
const run = async () => {
    for (const serverId of serverIds) {
        try {
            ...
            await uploadFile(serverId, path.join(__dirname, 'target', 'K-Teleport-1.0.jar'));
            await restart('K-Teleport', serverId)
