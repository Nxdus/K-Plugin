...
const reloadType = "plugman" // "restart" | "plugman"
const reloadCore = false
...
fs.watch(path.join(__dirname, 'target', 'K-Realms-1.0.jar'), (eventType, filename) => {
...
const run = async () => {
    for (const serverId of serverIds) {
        try {
            ...
            await uploadFile(serverId, path.join(__dirname, 'target', 'K-Realms-1.0.jar'));
            await restart('K-Realms', serverId)

            if(reloadCore) {
                await uploadFile(serverId, path.join(__dirname, 'libs', 'K-Core-1.0.jar'));
                await uploadFile(serverId, path.join(__dirname, 'libs', 'K-Teleport-API-1.0.jar'));
