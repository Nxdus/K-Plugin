...
fs.watch(path.join(__dirname, 'target', 'K-MythicCrucible-1.0.jar'), (eventType, filename) => {
...
const run = async () => {
    for (const serverId of serverIds) {
        try {
            ...
            await uploadFile(serverId, path.join(__dirname, 'target', 'K-MythicCrucible-1.0.jar'));
            restart('K-MythicCrucible', serverId)
        } catch (error) {
